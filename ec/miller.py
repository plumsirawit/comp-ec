...
def weil(P, Q):
    """
    Computes the Weil pairing for (P, Q).
    """
    # Find S in E not in the subgroup generated by P and Q
    subgroup = set()
    T = P
    while T not in subgroup:
        subgroup.add(T)
        U = T + Q
        while U not in subgroup:
            subgroup.add(U)
            U = U + Q
        T = T + P
    for x in range(P.field.card):
        ysq = -P.eqn.eval(P.field(x), P.field(0))
